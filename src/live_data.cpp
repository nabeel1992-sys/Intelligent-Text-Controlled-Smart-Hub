...
void blinkConfirm() {
    for (int i = 0; i < 2; i++) {
        digitalWrite(led, HIGH); // Active-High (ON)
        delay(100);
        digitalWrite(led, LOW);  // Active-High (OFF)
        delay(100);
    }
}
...
void webSocketEvent(uint8_t num, WStype_t type, uint8_t * payload, size_t length) {
    if (type == WStype_TEXT) {
        ...
        if (msg == "led_on") {
            digitalWrite(led, HIGH); // Turn LED ON (Active-High logic)
            broadcastLog("💡 Command received: LED ON");
        } 
        else if (msg == "led_off") {
            digitalWrite(led, LOW); // Turn LED OFF (Active-High logic)
